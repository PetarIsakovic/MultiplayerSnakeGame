                ...
                firebase.auth().onAuthStateChanged((user) => {
                    ...
                    if (user) {
                        ...
                        allPlayersRef.on("value", (snapshot) => {
                            ...
                            Object.keys(players).forEach(key => {
                                if (players[key].queued) {

                                    let allChildren = document.getElementById("queueWait").childNodes;
                                    let childCount = 2;
                                    for (let i = 2; i < allChildren.length; i++) {
                                        if (allChildren[i].id != players[key].id) {
                                            childCount++;
                                        }
                                    }
                                    if (childCount == allChildren.length) {

                                        const playerName = document.createElement("h1");
                                        playerName.classList.add("playerListName")
                                        playerName.innerText = "Player: " + (allChildren.length - 1) + ": " + players[key].name;
                                        playerName.id = players[key].id;
                                        queueWait.appendChild(playerName);
                                    }
                                }
                                else {
                                    let allChildren = document.getElementById("queueWait").childNodes;
                                    let parent = document.getElementById("queueWait");
                                    for (let i = 2; i < allChildren.length; i++) {
                                        if (allChildren[i].id == players[key].id) {
                                            parent.removeChild(allChildren[i]);
                                            break;
                                        }
                                    }
